...
const ChatMessage = ({ message, onPromoteToTask, onSaveMessage, isStreaming = false, assistantName = 'Assistant' }) => {
  ...
  if (type === 'codex_run') {
    ...
    return (
      <div className={`flex animate-slide-up justify-start mb-2 group`}>
        <div className="max-w-[900px] p-3 rounded-lg shadow bg-brand-surface-bg text-brand-text-primary rounded-bl-none">
  ...
  return (
    <div className={`flex animate-slide-up ${isUser ? 'justify-end' : 'justify-start'} mb-2 group`}>
      <div
        className={`max-w-[900px] p-3 rounded-lg shadow ${
